  ...
  <div class="min-h-screen bg-gray-50 p-8">
    <div class="max-w-5xl mx-auto">
      ...
      <div class="flex justify-between items-center mb-8">
        <div>
          ...
          <p class="text-gray-500 mt-1">工业级全栈旅行规划助手</p>
        </div>
        <button 
        ...
        >
          ...
          <div class="mt-4 space-y-2 text-gray-600 text-sm">
            <p class="flex items-center">
              <span class="w-5">📍</span> {{ trip.destination || '未定目的地' }}
            </p>
            <p class="flex items-center">
              <span class="w-5">📅</span> {{ new Date(trip.startDate).toLocaleDateString() }} 出发
            </p>
          </div>
        </div>
      </div>
      ...
      <div v-if="trips.length === 0" class="text-center py-20 bg-white rounded-xl border border-dashed border-gray-300 mt-6">
        <p class="text-gray-400">还没有行程，快去创建一个吧！🚀</p>
      </div>
    ...
    <div v-if="showModal" class="fixed inset-0 bg-black/50 flex items-center justify-center backdrop-blur-sm z-50">
      <div class="bg-white rounded-xl shadow-2xl w-full max-w-md p-6 transform transition-all">
        <h2 class="text-2xl font-bold text-gray-800 mb-6">创建新行程</h2>
        
        <form @submit.prevent="createTrip" class="space-y-4">
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">行程标题</label>
            <input v-model="form.title" type="text" placeholder="例如：日本樱花之旅" class="w-full border rounded-lg p-2 focus:ring-2 focus:ring-blue-500 outline-none" required />
          </div>

          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">目的地</label>
            <input v-model="form.destination" type="text" placeholder="例如：Tokyo" class="w-full border rounded-lg p-2 focus:ring-2 focus:ring-blue-500 outline-none" />
          </div>
          ...
          <div class="flex justify-end gap-3 mt-8">
            ...
            <button type="submit" class="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">立即创建</button>
          </div>
        </form>
      </div>
    </div>
